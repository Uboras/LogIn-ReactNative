...
//accion para cambiar la visibilidad de la contraseña
  const toggleShowPassword = () => {
    setShowPassword(!showPassword);
  };
//inicio autimatico si hay un usuario ya registrado
  ...
  //verificacion de inicio de sesion simple. 
  const verificacion = () => {
      return (
      User === userMoment &&
      Pass === passMoment &&
...
//limpar campos input user y password
  const limpiarCampos = () => {
    setPassMoment("");
    setUserMoment("");
  };
//accion de inicio correcto redireccionamiento a Home
  const handleLogin = () => {
